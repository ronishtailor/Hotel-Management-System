...
public class FrmAddEditBooking extends JInternalFrame {
	...
	FrmAddEditBooking(FrmBooking frmBooking,String query,JDesktopPane jdp){
		...
		jcbRoomNo=new JComboBox<>(CommonMethods.getAllRoomNos(" where roomid=1"));
		...
		if(query.equals("")){
			...
			try{
				...
				jcbCategory.setSelectedItem(rst.getString(8));
		...
		btnSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae){
				...
				DConnection dc=new DConnection();
				if(query.equals(""))
					dc.executeOtherQuery("insert into bookings values("+s1+",'"+s2+"','"+s3+"',"+s4+",'"+s5+"',"+s6+","+s7+","+catid+","+s9+","+s10+","+s11+","+s12+","+s13+")");
				else
					dc.executeOtherQuery("update bookings set checkindate='"+s3+"',days="+s4+",checkoutdate='"+s5+"',adults="+s6+",kids="+s7+",catid="+catid+",roomnumber="+s9+",rent="+s10+",guestid="+s11+",estimate="+s12+",advance="+s13+" where bookingid="+s1);
